...
router.post(
  "/rfp-sources/crawler/discovery-candidates/review",
  async (req, res) => {
    ...
    try {
      ...
      const configId = `public-portal:${candidate.sourceId}`;
      if (candidate.approvedConfig) {
        registerSpiderConfig(candidate.approvedConfig);
      } else {
        unregisterSpiderConfig(configId);
      }
