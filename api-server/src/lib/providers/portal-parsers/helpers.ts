...
export function objectValue(record: Record<string, unknown>, keys: string[]): unknown {
  ...
  const found = Object.entries(record).find(([key]) => normalizedKeys.includes(normalizeKey(key)));
  return found?.[1];
}

export interface CandidateFieldMap {
  title?: string[];
  sourceUrl?: string[];
  agency?: string[];
...
export function candidatesFromRecords(records: unknown[], sourceId: string, state: string | undefined, baseUrl?: string, fieldMap: CandidateFieldMap = {}): PortalCandidateOpportunity[] {
  return records.flatMap((raw) => {
    if (!raw || typeof raw !== "object") return [];
    const record = raw as Record<string, unknown>;
    const title = cleanupTitle(objectValue(record, [...(fieldMap.title ?? []), "title", "name", "description", "eventTitle", "bidTitle", "solicitationTitle"]));
    const description = cleanupTitle(objectValue(record, [...(fieldMap.description ?? []), "summary", "description", "abstract", "details"]));
    const sourceUrl = normalizeUrl(objectValue(record, [...(fieldMap.sourceUrl ?? []), "url", "link", "sourceUrl", "href", "detailUrl"]), baseUrl);
    const text = textFromUnknown(raw);
    if (!title || isJunkText(text)) return [];
    return [{
      title,
      sourceUrl,
      agency: cleanupTitle(objectValue(record, [...(fieldMap.agency ?? []), "agency", "buyer", "department", "organization", "owner"])),
      solicitationNumber: cleanupTitle(objectValue(record, [...(fieldMap.solicitationNumber ?? []), "solicitationNumber", "bidNumber", "eventId", "eventNumber", "id"])) ?? extractSolicitationNumber(text),
      postedDate: extractDate(objectValue(record, [...(fieldMap.postedDate ?? []), "postedDate", "publishDate", "issueDate", "startDate"]) ?? text, ["posted", "published", "issue date", "start date", "publication date", "advertisement date"]),
      responseDeadline: extractDeadline(objectValue(record, [...(fieldMap.responseDeadline ?? []), "responseDeadline", "deadline", "closingDate", "dueDate", "endDate"]) ?? text),
      description,
      location: cleanupTitle(objectValue(record, [...(fieldMap.location ?? []), "location", "placeOfPerformance", "state"])),
      state,
