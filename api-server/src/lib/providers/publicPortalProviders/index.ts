...
const DEDICATED_SOURCE_IDS = new Set(listRegisteredPublicPortalAdapterIds());
const ALWAYS_RUN_SOURCE_IDS = new Set(
  Array.from(DEDICATED_SOURCE_IDS).filter(
    (sourceId) => !CIVICENGAGE_PORTAL_IDS.has(sourceId),
  ),
);
...
function sourceIdForRecord(record: NormalizedOpportunity): string | undefined {
  const value =
    record.rawData?.sourceId ?? record.rawData?.parsedPortalSourceId;
...
async function hydrateSourceStatuses(): Promise<void> {
  ...
  for (const [sourceId, status] of persisted)
...
async function waitForDomainRateLimit(
  domain: string,
  signal?: AbortSignal,
): Promise<void> {
  if (signal?.aborted)
    throw abortError(`Rate limit wait for ${domain}`, signal);
  const lastFetchAt = lastDomainFetchAt.get(domain) ?? 0;
  const waitMs = Math.max(
    0,
    MIN_DOMAIN_INTERVAL_MS - (Date.now() - lastFetchAt),
  );
  if (waitMs > 0) {
  ...
  try {
    const response = await fetch(pageUrl, {
      ...
      headers: {
        ...
        "user-agent":
          "OccuMed-InsightHub/1.0 public procurement catalog crawler (+https://www.occumed.com)",
      },
    });
    if (!response.ok)
      throw new Error(
        `${source.id} returned HTTP ${response.status} for ${pageUrl}`,
      );
    ...
    if (requestSignal.signal.aborted) {
      ...
      if (reason instanceof DOMException && reason.name === "TimeoutError") {
        throw new Error(
          `${source.id} timed out after ${timeoutMs}ms for ${pageUrl}`,
        );
      }
      if (reason instanceof Error) throw reason;
...
async function runExistingParser(
  source: PublicPortalSource,
  options: FetchOptions,
): Promise<NormalizedOpportunity[]> {
  const adapter = getRegisteredPublicPortalAdapter(source.id);
  if (adapter)
    return (await adapter.fetch(options)).records.map((record) =>
      withPublicPortalMetadata(record, source),
    );
  throw new Error(
    `No dedicated source adapter is registered for public portal source ${source.id}`,
  );
}
...
async function runPaginatedHtmlSource(
  ...
  while (
    ...
    const pageRecords = extractor(
      html,
      pageSource,
      Math.max(1, limit - records.length),
    );
    ...
    for (const nextUrl of nextUrls) {
      ...
      if (
        !seenPages.has(nextKey) &&
        !queue.some((queued) => normalizedPageUrl(queued) === nextKey)
      ) {
        queue.push(nextUrl);
      }
    }
...
async function runSource(
  ...
  if (getRegisteredPublicPortalAdapter(source.id))
    return runExistingParser(source, { ...options, limit });
  if (source.scraperType === "existing_parser")
    return runExistingParser(source, { ...options, limit });
  if (source.scraperType === "static_html") {
    return runPaginatedHtmlSource(
      source,
      limit,
      timeoutMs,
      maxPages,
  ...
  throw new Error(
    `Source ${source.id} does not have a runnable direct or generic public-page adapter`,
  );
}
...
export class PublicPortalProvidersProvider implements DataSourceProvider {
  ...
  constructor(
    private readonly sources: PublicPortalSource[] = PUBLIC_PORTAL_SOURCES,
  ) {}
  ...
  async fetch(options: FetchOptions): Promise<ProviderFetchResult> {
    ...
    try {
      ...
      errors.push(
        `portal-health-load: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    const sourceTimeoutMs = positiveIntegerEnv(
      "PUBLIC_PORTAL_SOURCE_TIMEOUT_MS",
      DEFAULT_SOURCE_TIMEOUT_MS,
      5_000,
      120_000,
    );
    const runTimeoutMs = positiveIntegerEnv(
      "PUBLIC_PORTAL_RUN_TIMEOUT_MS",
      DEFAULT_RUN_TIMEOUT_MS,
      15_000,
      300_000,
    );
    const concurrency = positiveIntegerEnv(
      "PUBLIC_PORTAL_CONCURRENCY",
      DEFAULT_CONCURRENCY,
      1,
      10,
    );
    const maxPages = positiveIntegerEnv(
      "PUBLIC_PORTAL_MAX_PAGES",
      DEFAULT_MAX_PAGES,
      1,
      10,
    );
    ...
    await runWithConcurrency(
      ...
      async (source) => {
        ...
        if (remainingMs <= 0) {
          errors.push(
            `${source.id}: provider run deadline reached before this source started; it remains first in the next rotation`,
          );
          return;
        }
    ...
    if (remainingForDiscovery <= 0) {
      errors.push(
        "serper-official-portal-discovery: skipped because the provider run deadline was reached",
      );
      ...
      try {
        ...
        const sourceById = new Map(
          this.sources.map((source) => [source.id, source]),
        );
        const seen = new Set(records.map(opportunityKey));
        const discoveredRecords =
          publicPortalDiscovery.toOpportunities(discoveredPages);
  ...
  async getStatus(): Promise<ProviderStatus> {
    ...
    const lastSuccess = statuses.reduce<Date | undefined>(
      (latest, status) =>
        status.lastSuccessAt && (!latest || status.lastSuccessAt > latest)
    ...
    return {
      ...
      recordCount: statuses.reduce(
        (sum, status) => sum + status.resultCount,
        0,
      ),
      lastAttempt,
      lastSuccess,
...
export const publicPortalProvidersProvider =
  new PublicPortalProvidersProvider();
export * from "./catalog";
