...
import { publicPortalProvidersProvider as catalogPortalProvider, type PublicPortalSourceRunStatus } from "./publicPortalProviders/index";
...
const dedicatedGroups: DedicatedGroup[] = [
  ...
  { sources: STATEWIDE_PROCUREMENT_SOURCES, providers: statewideProcurementProviders, statuses: new Map() },
];

async function hydrateDedicatedStatuses(): Promise<void> {
  const persisted = await loadPublicPortalHealth();
  for (const group of dedicatedGroups) for (const source of group.sources) { const status = persisted.get(source.id); if (status) group.statuses.set(source.id, status); }
}
...
async function saveDedicatedStatus(status: PublicPortalSourceRunStatus, target: Map<string, PublicPortalSourceRunStatus>, errors: string[]): Promise<void> {
  ...
  try { await savePublicPortalHealth(status); } catch (error) { errors.push(`${status.sourceId}: portal health persistence failed: ${error instanceof Error ? error.message : String(error)}`); }
}
...
function recordKey(record: NormalizedOpportunity): string {
  ...
  if (record.sourceUrl) try { const url = new URL(record.sourceUrl); url.hash = ""; return `url:${url.toString().toLowerCase()}`; } catch { return `url:${record.sourceUrl.toLowerCase()}`; }
...
async function runDedicatedSource(source: PublicPortalSource, provider: DataSourceProvider | undefined, statuses: Map<string, PublicPortalSourceRunStatus>, options: FetchOptions, limit: number): Promise<ProviderFetchResult> {
  ...
  try {
    ...
    const status = result.records.length === 0 && result.errors.length > 0 ? failedPortalStatus(source, prior, checkedAt, result.errors.join("; ")) : successfulPortalStatus(source, prior, new Date(), result.records.length, 0);
...
class CombinedPublicPortalProvider implements DataSourceProvider {
  ...
  async isConfigured(): Promise<boolean> {
    return (await catalogPortalProvider.isConfigured().catch(() => false)) || dedicatedGroups.some((group) => group.sources.some((source) => Boolean(group.providers[source.id])));
  }

  getSources(): PublicPortalSource[] { return mergedSources(); }
  ...
  async fetch(options: FetchOptions): Promise<ProviderFetchResult> {
    ...
    const sourceCount = Math.max(dedicatedGroups.reduce((sum, group) => sum + group.sources.length, 0), 1);
    const perSource = Math.max(1, Math.ceil(limit / sourceCount));
    const errors: string[] = [];
    try { await hydrateDedicatedStatuses(); } catch (error) { errors.push(`dedicated-portal-health-load: ${error instanceof Error ? error.message : String(error)}`); }
    const tasks = dedicatedGroups.flatMap((group) => group.sources.map((source) => runDedicatedSource(source, group.providers[source.id], group.statuses, options, perSource)));
    const settled = await Promise.allSettled([...tasks, catalogPortalProvider.fetch({ ...options, limit })]);
    const candidates: NormalizedOpportunity[] = [];
    for (const result of settled) {
      if (result.status === "fulfilled") { candidates.push(...result.value.records); errors.push(...result.value.errors); }
      else errors.push(result.reason instanceof Error ? result.reason.message : String(result.reason));
    }
    const seen = new Set<string>();
    const records = candidates.filter((record) => { const key = recordKey(record); if (seen.has(key)) return false; seen.add(key); return true; }).slice(0, limit);
  ...
  async getStatus(): Promise<ProviderStatus> {
    ...
    return {
      ...
      errorMessage: [base?.errorMessage, failures.length ? `${failures.length} dedicated portal${failures.length === 1 ? " is" : "s are"} currently failing` : undefined].filter(Boolean).join("; ") || undefined,
