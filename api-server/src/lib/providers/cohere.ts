...
export class CohereProvider implements DataSourceProvider {
  ...
  async rerank(
    ...
    if (documents.length === 0 || !(await credentials.isConfigured()))
      return null;

    try {
      return await credentials.run(async (apiKey) => {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Cohere rerank HTTP ${response.status}: ${body.slice(0, 200)}`,
          );
        }
      ...
      console.warn(
        `[Cohere rerank] ${err instanceof Error ? err.message : err}`,
      );
      return null;
    }
  }
