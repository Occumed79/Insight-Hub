...
export async function runRfpStartupMigrations(): Promise<void> {
  ...
  try {
    await db.transaction(async (tx) => {
      // Render can overlap old/new processes during deploys. Serialize the entire
      // startup migration contract so two instances cannot race DDL/backfills.
      await tx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtextextended('insight-hub-rfp-startup-migrations', 0))`,
      );
      ...
      await tx.execute(
        sql`ALTER TYPE opportunity_ingestion_run_status ADD VALUE IF NOT EXISTS 'cancelled'`,
      );
      await tx.execute(
        sql`ALTER TYPE opportunity_ingestion_source_status ADD VALUE IF NOT EXISTS 'timed_out'`,
      );
      await tx.execute(
        sql`ALTER TYPE opportunity_ingestion_source_status ADD VALUE IF NOT EXISTS 'cancelled'`,
      );
      await tx.execute(sql`
