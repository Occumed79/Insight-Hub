...
const DISCOVERY_ROLES = ["direct_source", "browser_discovery", "intelligence"] as const;

test("source architecture has one valid owner for every active integration", () => {
...
test("manual RFP runtime excludes disabled crawler and browser automation paths", () => {
  ...
  for (const provider of ["samGov", "tango", "rssAggregator", "emailNotifications"]) {
    assert.equal(
  ...
  assert.throws(() =>
    architecture.assertSourceAllowedForRoles("selfHostedCrawler", DISCOVERY_ROLES),
  );
  assert.throws(() =>
...
test("provider budget policy enforces configured daily monthly and reserve ceilings", () => {
  ...
  try {
    ...
    if (oldDaily === undefined) delete process.env.INSIGHT_BUDGET_LANGSEARCH_PRIMARY_DAILY;
    else process.env.INSIGHT_BUDGET_LANGSEARCH_PRIMARY_DAILY = oldDaily;
    if (oldMonthly === undefined) delete process.env.INSIGHT_BUDGET_LANGSEARCH_PRIMARY_MONTHLY;
    else process.env.INSIGHT_BUDGET_LANGSEARCH_PRIMARY_MONTHLY = oldMonthly;
    if (oldReserve === undefined) delete process.env.INSIGHT_BUDGET_LANGSEARCH_PRIMARY_RESERVE;
    else process.env.INSIGHT_BUDGET_LANGSEARCH_PRIMARY_RESERVE = oldReserve;
  }
});
