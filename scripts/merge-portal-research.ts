...
const relevanceRecords = DIRECT_RFP_PORTALS.map((portal) => {
  ...
  const categories = [...new Set(evidence.flatMap((item) => item.matchedServiceCategories))];
  const completedDate = research.researchCompletedAt.slice(0, 10);
  if (research.researchStatus === "verified_relevant") {
    const reviewMethod = evidence.some((item) => item.evidenceType === "award_or_contract")
      ? "official_contract_history"
      : evidence.some((item) => item.evidenceType === "forecast_or_sources_sought")
  ...
  if (research.researchStatus === "inaccessible") {
    return {
      ...
      relevanceEvidence: [
        `The portal could not be meaningfully reviewed after ${research.queriesExecuted.length} official-domain searches. Errors: ${research.errors.slice(0, 3).join(" | ") || "portal and search access unavailable"}.`,
      ],
      relevanceEvidenceUrls: [],
      lastRelevanceVerified: completedDate,
  ...
  return {
    ...
    relevanceEvidence: [
      `Completed ${research.queriesExecuted.length} official-domain searches and inspected ${research.officialPagesInspected.length} official pages; no qualifying Occu-Med procurement artifact met the evidence rules.`,
    ],
    relevanceEvidenceUrls: [],
    lastRelevanceVerified: completedDate,
