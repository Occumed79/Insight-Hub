...
const REQUIRED_INDEXES = [
  ...
  ["opportunity_ingestion_run_sources", "uq_opportunity_ingestion_run_source"],
];
...
try {
  ...
  const runSourceIndex = indexMap.get(
    "opportunity_ingestion_run_sources:uq_opportunity_ingestion_run_source",
  );
  if (
    runSourceIndex &&
    (!/UNIQUE/i.test(runSourceIndex) ||
      !/run_id/i.test(runSourceIndex) ||
      !/provider/i.test(runSourceIndex))
  ) {
    failures.push(
      "uq_opportunity_ingestion_run_source no longer enforces run_id + provider uniqueness",
    );
  }
